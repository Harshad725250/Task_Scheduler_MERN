  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      tasks.forEach((task) => {
        if (task.reminder && !task.completed) {
          ...
          if (isPast(reminderTime) && !newAlerts.has(task._id)) {
            window.alert(`Reminder: ${task.title} is due!\nDue: ${format(reminderTime, "PPpp")}`);
      ...
      if (newAlerts.size !== alertedReminders.size) {
        ...
        sessionStorage.setItem("alertedReminders", JSON.stringify(Array.from(newAlerts)));
      }
    };
    ...
    try {
      ...
      if (isEditing && currentTaskId) {
        ...
        setAlertedReminders((prev) => {
          ...
          sessionStorage.setItem("alertedReminders", JSON.stringify(Array.from(newSet)));
          return newSet;
        });
      } else {
